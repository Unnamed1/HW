<?php  return '$result=$modx->query(\'SELECT `modx_site_content`.`id`, `pagetitle`, `content`, `publishedon`, `uri`, `value` FROM `modx_site_content` JOIN `modx_site_tmplvar_contentvalues` ON `modx_site_content`.`id`=`contentid` WHERE `tmplvarid`=2 AND `value` LIKE \\\'%\'.$_GET[\'tag\'].\'%\\\' ORDER BY `publishedon` DESC\')->fetchAll(PDO::FETCH_ASSOC);
$return=\'\';
