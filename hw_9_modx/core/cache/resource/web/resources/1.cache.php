<?php  return array (
  ...
  array (
    ...
    'cacheable' => 1,
    'createdby' => 1,
    'createdon' => 1473858080,
    'editedby' => 1,
    'editedon' => 1473875460,
    'deleted' => 0,
    'deletedon' => 0,
    'deletedby' => 0,
    'publishedon' => 1473875400,
    'publishedby' => 0,
    'menutitle' => '',
    'donthit' => 0,
    'privateweb' => 0,
    ...
    array (
      ...
      array (
        ...
        array (
          ...
          'category' => 0,
          'cache_type' => 0,
          'snippet' => '$result=$modx->query(\'SELECT `id`, `pagetitle`, `content`, `publishedon`, `uri` FROM `modx_site_content`WHERE `parent`!=0 AND `isfolder`=0 ORDER BY `publishedon` DESC\');
$return=\'\';
...
return $return;',
          ...
          'static' => false,
          'static_file' => '',
          'content' => '$result=$modx->query(\'SELECT `id`, `pagetitle`, `content`, `publishedon`, `uri` FROM `modx_site_content`WHERE `parent`!=0 AND `isfolder`=0 ORDER BY `publishedon` DESC\');
$return=\'\';
while($res=$result->fetch(PDO::FETCH_ASSOC)){
    $tmp=preg_split(\'/[^\\p{L}\\p{N}\\\']+/u\', $res[\'content\']);
    $limit=count($tmp)>=50?50:count($tmp);
