<?php  return array (
  ...
  array (
    ...
    'cacheable' => 1,
    'createdby' => 1,
    'createdon' => 1474026287,
    'editedby' => 1,
    'editedon' => 1474026305,
    'deleted' => 0,
    'deletedon' => 0,
    'deletedby' => 0,
    'publishedon' => 1474026305,
    'publishedby' => 1,
    'menutitle' => '',
    'donthit' => 0,
    'privateweb' => 0,
    ...
    exit();
      ...
      array (
        ...
        array (
          ...
          'category' => 0,
          'cache_type' => 0,
          'snippet' => '$result=$modx->query(\'SELECT `modx_site_content`.`id`, `pagetitle`, `content`, `publishedon`, `uri`, `value` FROM `modx_site_content` JOIN `modx_site_tmplvar_contentvalues` ON `modx_site_content`.`id`=`contentid` WHERE `tmplvarid`=2 AND `value` LIKE \\\'%\'.$_GET[\'tag\'].\'%\\\' ORDER BY `publishedon` DESC\')->fetchAll(PDO::FETCH_ASSOC);
$return=\'\';
...
return $return;',
          ...
          'static' => false,
          'static_file' => '',
          'content' => '$result=$modx->query(\'SELECT `modx_site_content`.`id`, `pagetitle`, `content`, `publishedon`, `uri`, `value` FROM `modx_site_content` JOIN `modx_site_tmplvar_contentvalues` ON `modx_site_content`.`id`=`contentid` WHERE `tmplvarid`=2 AND `value` LIKE \\\'%\'.$_GET[\'tag\'].\'%\\\' ORDER BY `publishedon` DESC\')->fetchAll(PDO::FETCH_ASSOC);
$return=\'\';
foreach($result as $post){
    $tags=explode(\',\', $post[\'value\']);
    $num=count($tags);
