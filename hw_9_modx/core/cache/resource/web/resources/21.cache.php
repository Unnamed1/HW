<?php  return array (
  ...
  array (
    ...
    'cacheable' => 1,
    'createdby' => 1,
    'createdon' => 1474115942,
    'editedby' => 1,
    'editedon' => 1474115958,
    'deleted' => 0,
    'deletedon' => 0,
    'deletedby' => 0,
    'publishedon' => 1474115958,
    'publishedby' => 1,
    'menutitle' => '',
    'donthit' => 0,
    'privateweb' => 0,
...
',
    ...
    array (
      ...
      array (
        ...
        array (
          ...
          'source' => 1,
          'property_preprocess' => false,
          'name' => 'getDate calendar',
          'description' => '',
          'editor_type' => 0,
          'category' => 0,
          'cache_type' => 0,
          'snippet' => 'if(isset($_GET[\'arc_year\']) && isset($_GET[\'arc_month\'])){
    ...
    exit();
          ...
          'static' => false,
          'static_file' => '',
          'content' => 'if(isset($_GET[\'arc_year\']) && isset($_GET[\'arc_month\'])){
    if(empty($_GET[\'arc_year\']) || empty($_GET[\'arc_month\'])){
        header(\'Location: /\');
        exit();
    }
    ...
    exit();
      ...
      array (
        ...
        array (
          ...
          'source' => 1,
          'property_preprocess' => false,
          'name' => 'getPosts calendar',
          'description' => '',
          'editor_type' => 0,
          'category' => 0,
          'cache_type' => 0,
          'snippet' => '$result=$modx->query(\'SELECT `id`, `pagetitle`, `content`, `publishedon`, `uri` FROM `modx_site_content` WHERE FROM_UNIXTIME(`publishedon`, \\\'%m %Y\\\')=\\\'\'.$_GET[\'arc_month\'].\' \'.$_GET[\'arc_year\'].\'\\\' AND `parent`!=0 AND `isfolder`=0 ORDER BY `publishedon` DESC\')->fetchAll(PDO::FETCH_ASSOC);
$return=\'\';
...
return $return;',
          ...
          'static' => false,
          'static_file' => '',
          'content' => '$result=$modx->query(\'SELECT `id`, `pagetitle`, `content`, `publishedon`, `uri` FROM `modx_site_content` WHERE FROM_UNIXTIME(`publishedon`, \\\'%m %Y\\\')=\\\'\'.$_GET[\'arc_month\'].\' \'.$_GET[\'arc_year\'].\'\\\' AND `parent`!=0 AND `isfolder`=0 ORDER BY `publishedon` DESC\')->fetchAll(PDO::FETCH_ASSOC);
$return=\'\';
foreach($result as $post){
    $tmp=preg_split(\'/[^\\p{L}\\p{N}\\\']+/u\', $post[\'content\']);
    $limit=count($tmp)>=50?50:count($tmp);
